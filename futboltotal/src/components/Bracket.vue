...
function showNotification(name, position) {
  ...
  // Burst inmediato para feedback instantáneo
  try {
    emitConfetti({
      particleCount: 16,
      angle: 90,
  ...
  // Confeti continuo mientras la ventana esté abierta
  if (notificationConfettiInterval.value) {
    try { clearInterval(notificationConfettiInterval.value) } catch (e) {}
  }
...
function computePositions() {
  ...
  ;['left', 'right'].forEach((side) => {
    ...
    // Match 0 (winner of 0 vs winner of 1): between rows 1 and 5 → center at row 3
    // Match 1 (winner of 2 vs winner of 3): between rows 9 and 13 → center at row 11
    if (rounds.length > 1) {
      positions[1] = [2, 10]
    }

    // THIRD ROUND (semis): Manually positioned to be centered
    // Match 0 (winner of cuarto 0 vs winner of cuarto 1): between rows 3 and 11 → center at row 7
    if (rounds.length > 2) {
      positions[2] = [3]
    }
...
function drawSideLines(ctx, container, side) {
  ...
  for (let rIdx = 0; rIdx < rounds.length - 1; rIdx++) {
    ...
    for (let mIdx = 0; mIdx < matchEls.length; mIdx++) {
      ...
      // Determinar el índice del padre (cada 2 matches van a 1 padre)
  ...
  const parentY = parentRect.top + parentRect.height / 2 - containerRect.top
      ...
      // Si es el primer match del par (índice par), dibujar línea vertical al siguiente
      if (mIdx % 2 === 0 && matchEls[mIdx + 1]) {
        ...
        // Línea en forma de bracket conectando el par
        ctx.beginPath()
        ctx.moveTo(x, y)
        ...
        // Línea del punto medio al padre
        ctx.beginPath()
        ctx.moveTo(x + extendX, midY)
        ctx.lineTo(parentX, parentY)
        ctx.stroke()
      } 
      // Si es el segundo del par (índice impar), ya se dibujó con el anterior
      else if (mIdx % 2 === 1) {
        // Skip, ya se dibujó junto con el match anterior
      }
      // Si no hay siguiente match (último match impar), conectar directo al padre
      else {
        ctx.beginPath()
        ctx.moveTo(x, y)
        ctx.lineTo(x + extendX, y)
  ...
  <Transition name="win-fade">
    <div v-if="winDialog.show" class="win-overlay" aria-live="assertive">
      <div class="win-card">
        ...
        <div class="win-loser">Venció a: <span class="loser-name">{{ winDialog.loser }}</span></div>
      </div>
    </div>
  </Transition>
  ...
  <Transition name="modal-fade">
    <div v-if="finalModalShow" class="final-modal-overlay" @click="closeFinalModal">
      <div class="final-modal" :class="{ running: finalMatchRunning }" @click.stop>
        ...
        <div v-if="finalMatchRunning" class="match-ball">⚽</div>

        <!-- Animated soccer balls -->
        <div class="soccer-ball ball-1">⚽</div>
        <div class="soccer-ball ball-2">⚽</div>
        <div class="soccer-ball ball-3">⚽</div>
        <div class="soccer-ball ball-4">⚽</div>
